...
1. **shortName mode** (default) – parameters are identified by their GRIB
   shortName (e.g. ``u``, ``v``).  Grid shape and chunk layout are taken from
   the module-level constants ``SHAPE`` / ``CHUNKS``.

2. **config mode** (``--config``) – a YAML configuration file describes the
   full CF-conventions dataset structure.  Variables are identified by their
   GRIB2 discipline / parameterCategory / parameterNumber keys, and the
   dataset shape is derived from the coordinate definitions in the config.
...
def initialise_zarr_from_config(zarr_path: str, config: dict) -> xr.Dataset:
...
async def read_grib_config(grib_file_path: str, matchers: list):
    """Async generator yielding matched GRIB messages for config-based operation.
...
async def write_slice_config(
    ...
    """Write a single level slice for a config-based variable into the Zarr store.
    ...
    ----------
    ds:
        In-memory dataset produced by :func:`initialise_zarr_from_config`.
    var_name:
...
async def producer_config(
    queue: asyncio.Queue,
    grib_file_path: str,
    matchers: list,
) -> None:
    """Read GRIB messages in config mode and push them onto *queue*.
    ...
    matchers:
        List produced by :func:`_build_var_matcher`.
    """
    async for message in read_grib_config(grib_file_path, matchers):
        await queue.put(message)
    await queue.put(None)  # Sentinel


async def consumer_config(
    queue: asyncio.Queue,
    ds: xr.Dataset,
    zarr_path: str,
) -> None:
    """Consume config-mode messages from *queue* and write to Zarr.

    Parameters
    ----------
    queue:
        Shared asyncio queue populated by :func:`producer_config`.
    ds:
    ...
    while True:
        ...
        await write_slice_config(ds, var_name, t_idx, z_idx, values, zarr_path)
        queue.task_done()
...
async def main(
    ...
    params:
        List of GRIB shortNames to extract and store (shortName mode).
    config_path:
        Optional path to a YAML configuration file.  When provided, the
        dataset is built from the config and variables are matched by GRIB2
        keys instead of shortName.
    """
    if config_path is not None:
        config = load_config(config_path)
        ds = initialise_zarr_from_config(zarr_path, config)
        matchers = _build_var_matcher(config)
        var_names = [m[0] for m in matchers]

        queue: asyncio.Queue = asyncio.Queue()
        producer_task = asyncio.create_task(
            producer_config(queue, grib_file_path, matchers)
        )
        consumer_task = asyncio.create_task(consumer_config(queue, ds, zarr_path))
...
def _parse_args(argv=None):
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "When provided, variables are matched by GRIB2 keys "
            "(discipline/parameterCategory/parameterNumber) instead of "
            "shortName, and --params is ignored."
        ),
    )
...
if __name__ == "__main__":
    ...
    asyncio.run(main(args.grib_file, args.zarr_path, args.params, args.config))
